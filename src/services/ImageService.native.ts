...
class ImageProcessingServiceMobile {
  /**
  ...
  public async processImage(imageUri: string): Promise<string> {
    try {
      ...
      const cropResult: any = await ImageEditor.cropImage(imageUri, cropData);
      const croppedImageUri = typeof cropResult === 'string' ? cropResult : cropResult.uri;
  ...
  private async saveImageToGallery(tempUri: string): Promise<string> {
    try {
      ...
      Logger.error(error as Error, { message: '[Image Service] Erro ao salvar na galeria:' });
      return tempUri;
    }
  }
}
