...
class AuthTokenDao {
  ...
  public async saveCredentials(credential: Record<string, any>): Promise<void> {
    ...
    try {
      ...
      if (results[0].rows.length > 0) {
        ...
        await db.executeSql('UPDATE auth_tokens SET aditional = ?, active = ?, updated_at = ? WHERE platform = ?', [
          credential.aditional,
        ...
        await db.executeSql(
          'INSERT INTO auth_tokens (platform, aditional, active, created_at, updated_at) VALUES (?, ?, ?, ?, ?)',
          [credential.platform, credential.aditional, credential.active ? 1 : 0, createdAt, createdAt],
        );
      }
      ...
      Logger.error(error as Error, {
        message: `[Auth Token Dao] Erro ao salvar credenciais para ${credential.platform}:`,
      });
      throw error;
    }
  }
  ...
  public async getCredentialsForPlatform<T>(platform: PlatformType): Promise<T | null> {
    ...
    try {
      ...
      const results = await db.executeSql('SELECT platform, aditional, active FROM auth_tokens WHERE platform = ?', [
        platform,
      ]);
      if (results[0].rows.length > 0) {
  ...
  public async getAllCredentials(): Promise<Credentials[]> {
    ...
    try {
      const results = await db.executeSql(
        'SELECT platform, aditional, active FROM auth_tokens WHERE platform IS NOT NULL GROUP BY platform',
      );
  ...
  public async updateActiveStatus(credentials: Credentials): Promise<void> {
    ...
    try {
      ...
      Logger.info(
        `[Auth Token Dao] Status da plataforma ${credentials.platform} atualizado para ${credentials.active}`,
      );
    } catch (error) {
      Logger.error(error as Error, {
        message: `[Auth Token Dao] Erro ao atualizar status para ${credentials.platform}`,
      });
      throw error;
    }
  }
